...
export async function POST(request: Request) {
  try {
    ...
    const imageData = formData.get("imageUrl");

    if (!title || !slug || !content || !categoryId || !tagsJson) {
      return NextResponse.json(
    ...
    try {
      ...
      if (!Array.isArray(tags)) throw new Error("Tags is not a valid array.");
    } catch (e) {
      return NextResponse.json(
    ...
    if (imageData instanceof File) {
      // Real File object (uploaded file) — upload to Cloudinary
      try {
      ...
      // A string URL — just use it as is
      imageUrl = imageData;
    } else {
